---
sidebar_label: 'GTA/FiveM FivePD Controls'
sidebar_position: 2
---
...
```
	...
	Admin NOCLIP: F2
	Console: F8
	Toggle Weapon-in-Hand: Tab or [
	Chat Visiblity: L
	(Vehicle-Specific)Toggle Lights: H
	Enter/Exit Vehicle: F
	Controls: WASD
	(Vehicle-Specific)Handbrake: Space
	Look Behind: C
	Camera Mode Toggle: V
	Pause: Escape
	Player List (Also available in vMenu): Hold Up Arrow
...
```
	...
	Toggle emergency lights: Y
	Airhorn: E
	Toggle siren: Up Arrow
	Manual siren: R or .
...
```
	...
	Callout Menu: Z
	(Out-of-Vehicle-Specific)Ped Interation Menu: X
	ID Card Window: J
	Scene Management and Dispatch: G
	Waypoint closest Jail: O
	Unload prisoners to Jail at a Jail location: U
	(Out-of-Vehicle-Specific)Stop Ped: Hold E
	(Out-of-Vehicle-Specific)Threatenly stop Ped: Hold E and aim weapon
	Toggle ALPR (Automatic License Plate Recognition)[This will show any vehicle in bad standing]: F9
	(Vehicle-Specific)Blip vehicle for traffic stop: (With lights on) Left Shift (Hold to Cancel)
...
```
	Open Remote: F5 (Toggle Display to show radar, then press PWR to turn the radar, click the two XMT(ones with arrows in the center of the remote)
		...
		{[(You can drag the two panels around on the screen if you wish while the remote is open)]}
	Lock/unlock speed front antenna: NUM8
	Lock/unlock speed rear antenna: NUM5
