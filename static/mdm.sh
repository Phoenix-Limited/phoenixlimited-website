...
echo -e "${CYAN}*-------------------*--------------------*${NC}"
echo -e "${YELLOW}*         Check MDM - Skip MDM           *${NC}"
echo -e "${RED}*          phoenixlimited.net            *${NC}"
echo -e "${RED}*            Phoenix Limited             *${NC}"
echo -e "${CYAN}*-------------------*--------------------*${NC}"
echo ""

PS3='Please enter your choice: '
options=("Autobypass" "Check Enrollment" "Reboot" "Exit")
...
select opt in "${options[@]}"; do
	...
	"Autobypass")
		...
		if ! dscl -f "$dscl_path" localhost -list "$localUserDirPath" UniqueID | grep -q "\<$defaultUID\>"; then
			...
			echo -e "${CYAN}Enter Full Name (Default: Mac User)${NC}"
			read -rp "Full name: " fullName
			fullName="${fullName:="Mac User"}"

			echo -e "${CYAN}Username${NC} ${RED}WRITE WITHOUT SPACES (Default: macuser)${NC}"
			read -rp "Username: " username
			username="${username:=macuser}"
		...
		blockedDomains=("iprofiles.apple.com" "mdmenrollment.apple.com" "deviceenrollment.apple.com")
	...
	"Check Enrollment")
		...
		if ! sudo profiles show -type enrollment >/dev/null 2>&1; then
			echo -e "\n\t${GREEN}Not MDM locked / Script Successful${NC}\n"
		else
			echo -e "\n\t${RED}MDM locked / Script Failure${NC}\n"
		fi
		;;
